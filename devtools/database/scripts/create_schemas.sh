...
source devtools/database/setup/01-prepare_database.sh

# psql -h localhost dengue < crawlclima/utilities/dbschema/roles-infodengue_roles.sql0
# echo "[II] creating ${PG_RESTORE_STAGING} for the demo database."
psql --host "$PSQL_HOST"  -v ON_ERROR_STOP=1 --username "$POSTGRES_USER" -d ${PSQL_DB} < devtools/database/setup/00-infodengue_roles.sql
